namespace Scripts {
    export class PetCommander {
        ...
        static renameAndSavePet(petSerial:string):IMyPet {
            const petName = Scripts.PetCommander.getRandomAvailableName();
            const pet = {serial: petSerial, name: petName};
        ...
        static savePet(pet:{serial:string, name:string}) {
            const myPets = Scripts.PetCommander.getMyPets();
            myPets.push(pet);
            Shared.AddArray('myPets', myPets);
        }
        ...
        static getNewPet():IMyPet|undefined {
            ...
            for (const serial of monstersAlive) {
                ...
                if (isMyMonster) {
                    const pet = Scripts.PetCommander.renameAndSavePet(serial);
                    Orion.Ignore(serial);
                    return pet;
                }
            }
