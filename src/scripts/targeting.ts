        ...
        static targetNextMonster(
            ...
            if (timer < 0 || timer > timeToStorePreviousTargets) {
               ...
               const noto = notoriety.join('|') || undefined;
                ...
                const nearCharacters = Orion.FindTypeEx('any', '0xFFFF', 'ground', 'mobile|live|ignoreself', 18, noto).filter(a=>
                    !a.CanChangeName() && 
                    !friendList.some(f=>f === a.Serial())
            ...
            let result:TargetResult = new TargetResult();
